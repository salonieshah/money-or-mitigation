...
d3.csv('./database/deforest-global.csv', function(error, data) {
    ...
    }, {xAxis: 'YEARS', yAxis: 'DEFORESTATION (IN MILLION HECTARES)'});
    chart.bind("#deforest-line");
    chart.render();

});

d3.csv('./database/emission-global.csv', function(error, data) {
    data.forEach(function (d) {
        d.year = +d.year;
        d.FAO = +d.FAO;
        d.UNFCCC = +d.UNFCCC;
...
function makeLineChart(dataset, xName, yObjs, axisLabels) {
    ...
    var color = ["#FFC300", "#0C59F2", "#268C16"]
    ...
    chartObj.margin = {top: 15, right: 60, bottom: 60, left: 70};
...
// Render the chart
    chartObj.render = function () {
        ...
        chartObj.svg.append("g").attr("class", "y axis").call(chartObj.yAxis).append("text").attr("class", "label").attr("transform", "rotate(-90)").attr("y", -55).attr("x", -chartObj.height / 2).attr("dy", ".71em").style("text-anchor", "middle").text(chartObj.yAxisLabel);
